...
void Usuario::set_tag(string A){
    Tag = Tag + " " + A + " ";
}

string Usuario::get_Tag() const{
    return Tag;
}

Usuario::Usuario(string Email, string Senha, string Nome, string Data, string Endereco, string Telefone, string TelefoneF, 
string Documento, int Salario){

    set_email(Email);
    set_senha(Senha);
    set_nome_completo(Nome);
    set_data_nascimento(Data);
    set_endereco(Endereco);
    set_telefone(Telefone);
    set_telefone_familiar(TelefoneF);
